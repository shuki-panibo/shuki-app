    ...
    terms: {
      ...
      content: (
        <div className="space-y-6">
          ...
          <p className="leading-relaxed">本利用規約（以下「本規約」といいます）は、合同会社護己（以下「当社」といいます）が提供する「護己-Shuki-」（以下「本サービス」といいます）の利用条件を定めるものです。</p>
          <div>
            <h4 className="font-semibold text-base mb-2">第1条（適用）</h4>
            <p className="leading-relaxed mb-2">本規約は、本サービスの提供条件及び本サービスの利用に関する当社とユーザーとの間の権利義務関係を定めることを目的とし、ユーザーと当社との間の本サービスの利用に関わる一切の関係に適用されます。</p>
          </div>
          <div>
            <h4 className="font-semibold text-base mb-2">第2条（サービス内容）</h4>
            <p className="leading-relaxed mb-2">本サービスは、ユーザーの生活環境や家族構成などの診断結果に基づき、最適な防災備蓄プランを提案し、防災用品・食料品等をセットで販売するサービスです。</p>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="space-y-1 leading-relaxed">
              ...
              <p>• 公式LINE：<a href="https://lin.ee/v0KcwPS" className="text-orange-500 underline">https://lin.ee/v0KcwPS</a></p>
    ...
    privacy: {
      ...
      content: (
        <div className="space-y-6">
          ...
          <p className="leading-relaxed">合同会社護己は、「護己-Shuki-」における個人情報の取扱いについて、以下のとおり定めます。</p>
          <div>
      ...
      content: (
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="leading-relaxed mb-2">発送完了通知前までであれば、全額返金にて対応いたします。</p>
          </div>
        </div>
      )
    },
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-slate-50">
      <div className="bg-white shadow-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4">
          <button onClick={onBack} className="flex items-center gap-2 text-slate-600 hover:text-orange-500 transition-colors min-h-[48px]">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="text-center mb-6 sm:mb-8">
          ...
          <p className="text-sm sm:text-base text-slate-600">サービス利用に関する重要事項をご確認ください</p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-6 sm:mb-8">
          {Object.entries(policies).map(([key, policy]) => {
            const Icon = policy.icon;
            return (
              <button key={key} onClick={() => setActivePolicy(key)} className={`p-3 sm:p-4 rounded-xl font-medium transition-all min-h-[64px] sm:min-h-auto ${activePolicy === key ? 'bg-orange-500 text-white shadow-lg' : 'bg-white text-slate-700 hover:bg-orange-50'}`}>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-5 sm:p-8 md:p-12">
          <div className="flex items-center gap-3 mb-4 sm:mb-6">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-slate-800">{policies[activePolicy].title}</h2>
          </div>
          <div className="prose prose-slate max-w-none text-sm sm:text-base">{policies[activePolicy].content}</div>
        </div>
      </div>
    </div>
  );
